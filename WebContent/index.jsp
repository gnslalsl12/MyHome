...
<script>
  ...
  document.querySelector("#favorite-btn").addEventListener("click", ()=>{
	  ...
	  let url = "${root}/main?action=addFavorite&sido="+sido+"&gugun="+gugun+"&dong="+dong;
	  
	  fetch(url)
	  .then(response => sendRequest("favorite",false));
  ...
  function makeOutListener(map, infowindow, marker, apt, pos) {
    ...
    document.querySelector("#bestPath").addEventListener("click", ()=>{
	  ...
	  console.log(pos.Ma);
	  //창 띄우기
	  window.open("${root}/main?action=bestPath&dong="+dongName+"&La="+pos.La+"&Ma="+pos.Ma,"popup-test","width = 500, height = 500, top = 100, left = 200, location = no");
	//alert("click!!");
  });
  }
