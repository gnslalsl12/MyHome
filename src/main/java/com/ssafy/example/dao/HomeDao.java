...
import com.ssafy.example.util.DBUtil;
...
public class HomeDao {
	...
	public List<HomeDto> bestPath(String dongName, String dealYear, String dealMonth, String aptName) throws SQLException{
		...
		try {
			con = DBUtil.getInstance().getConnection();
			String sql = "select DISTINCT aptName, lat, lng from home where dongName=? and dealYear=? and dealMonth=? and aptName!=?";
			
			pstm= con.prepareStatement(sql);
			pstm.setString(1, dongName);
			pstm.setString(2, dealYear);
			pstm.setString(3, dealMonth);
			pstm.setString(4, aptName);
